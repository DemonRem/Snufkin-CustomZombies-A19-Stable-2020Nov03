================================================================================
3rd of November 2020 - Snufkin Custom Zombies (A19 - Stable) and back compatible
================================================================================
...
A replacement until such a time as an alternative can be coded is as follows:
- buffs.xml
<passive_effect name="HealthMax" operation="base_subtract" value="15"/> (This decreases base health by 15)
<triggered_effect trigger="onSelfBuffStart" action="ModifyCVar" cvar="$parasite" operation="set" value="5"/> (This gives a 5 second countdown until health is restored to normal values)
<triggered_effect trigger="onSelfBuffStack" action="ModifyCVar" cvar="$parasite" operation="add" value="10"/> (If you are hit for a second or more times while the intial 5 second countdown hasn't completed, a further 10 seconds are added to the countdown)

- items.xml
<property name="DamageEntity" value="10"/> (This value allows the Parasite melee attack to cause 8 damage in addition to the above loss of base health)

These new values offer a balance to new players and higher level players. The first hit that reduces 15 base health to low level players is on a 5 second countdown until the value can be restored. However, a second and further positive hit increases the countdown by incremental 10 second steps.

It would be wise for low level players to retreat at this point because restoring that 15 health when your base health is only between 100 to 125 on top of low natural healing and potentially limited bandages and first aid kits carries high risk of death. Furthermore, the Parasite has very strong restorative properties and the outcome is weighted towards her coming out victorious. For a higher level player, the opposite applies as health, restorative potential, weapon strength, and other factors can put the weighting towards a player victory. These values may need to be tweaked in the future, but playtesting has shown it to be balanced. 
------------------------
...
Commented out a random quest reward for Juggernaut quest completion as it does not appear to be pulled and is specifically not listed in dev notes on quest reward groups. This change brings it into line with the default settings.
